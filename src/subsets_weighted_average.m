function weighted_average_subsets(ref_fn_prefix, allmotl_fn_prefix,...
    weight_fn_prefix, iteration, class)
% WEIGHTED_AVERAGE_SUBSETS joins and weights subsets of average subsets.
%     WEIGHTED_AVERAGE_SUBSETS(
%         REF_FN_PREFIX,
%         MOTL_FN_PREFIX,
%         WEGIHT_FN_PREFIX,
%         ITERATION,
...
%
% Example:
%     WEIGHTED_AVERAGE_SUBSETS('./ref/ref', './combinedmotl/allmotl', ...
%         './otherinputs/wei', 3, 1)
%
...
%
% See also WEIGHTED_AVERAGE PARALLEL_SUMS_SUBSETS
...
%##############################################################################%
    ...
    % we have an equal number of averages and weights files
    num_avg_batches = length(dir(...
        sprintf('%s_%d_*_subset1.em', ref_fn_prefix, iteration)));
    ...
    num_subsets = length(dir(sprintf('%s_%d_1_subset*.em', ref_fn_prefix, ...
                                     iteration)));

    avg_sum_cell = {};
    weight_sum_cell = {};
    for subset_idx = 1:num_subsets
        avg_sum_cell{subset_idx} = getfield(tom_emread(sprintf(...
            '%s_%d_1_subset%d.em', ref_fn_prefix, iteration, subset_idx)), ...
            'Value');

        weight_sum_cell{subset_idx} = getfield(tom_emread(sprintf(...
            '%s_%d_1_subset%d.em', weight_fn_prefix, iteration, ...
            subset_idx)), 'Value');
    end
    ...
    for batch_idx = 2:num_batches
        for subset_idx = 1:num_subsets
            avg_sum_cell{subset_idx} =  avg_sum_cell{subset_idx} + ...
                ...
                ref_fn_prefix, iteration, batch_idx, subset_idx)), 'Value');
    ...
    for subset_idx = 1:num_subsets
        ...
        if iclass == 0
            % When the ptcl_idx is divisible by the subset number (minus one)
            % then it belongs to the subset
        ...
        average_fn = sprintf('%s_debug_raw_%d_subset%d.em', ref_fn_prefix,...
        ...
        average_fn = sprintf('%s_%d_subset%d.em', ref_fn_prefix, iteration,...
            subset_idx);
