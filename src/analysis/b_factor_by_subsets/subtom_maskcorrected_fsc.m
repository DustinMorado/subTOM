...
%##############################################################################%
    ...
    if exist(motl_a_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'maskcorrected_FSC:File %s does not exist.', motl_a_fn);
    ...
    if exist(motl_b_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'maskcorrected_FSC:File %s does not exist.', motl_b_fn);
    ...
    try
        validateattributes(iclass, {'numeric'}, ...
            {'scalar', 'nonnan', 'integer'}, ...
            'subtom_maskcorrected_FSC', 'iclass');
...
%##############################################################################%
    ...
    if size(motl_a, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'maskcorrected_FSC:%s is not proper MOTL.', motl_a_fn);
    ...
    if size(motl_b, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'maskcorrected_FSC:%s is not proper MOTL.', motl_b_fn);
    ...
    check_fn = sprintf('%s_%d_subset1.em', ref_a_fn_prefix, iteration);

    if exist(check_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'maskcorrected_FSC:File %s does not exist.', check_fn);
    ...
    for subset_idx = 1:num_subsets
        ...
        ref_a_fn = sprintf('%s_%d_subset%d.em', ref_a_fn_prefix, iteration, ...
            subset_idx);
        ...
        ref_b_fn = sprintf('%s_%d_subset%d.em', ref_b_fn_prefix, iteration, ...
            subset_idx);
    ...
    if do_sharpen == 1
        ...
        % Read in the references.
        ref_a_fn = sprintf('%s_%d_subset1.em', ref_a_fn_prefix, iteration);
        ref_a = getfield(tom_emread(ref_a_fn), 'Value');

        ref_b_fn = sprintf('%s_%d_subset1.em', ref_b_fn_prefix, iteration);
        ref_b = getfield(tom_emread(ref_b_fn), 'Value');
