function subtom_maskcorrected_fsc_bfactor(varargin)
% SUBTOM_MASKCORRECTED_FSC_BFACTOR "mask-corrected" FSC and sharpened refs.
%
%     SUBTOM_MASKCORRECTED_FSC_BFACTOR(
%         'ref_a_fn_prefix', REF_A_FN_PREFIX,
%         'ref_b_fn_prefix', REF_B_FN_PREFIX,
%         'motl_a_fn_prefix', MOTL_A_FN_PREFIX,
%         'motl_b_fn_prefix', MOTL_B_FN_PREFIX,
...
%         'iteration', ITERATION)
%
%     Takes in two references REF_A_FN and REF_B_FN and a FSC mask FSC_MASK_FN
%     and calculates a "mask-corrected" FSC. This works by randomizing the
%     structure factor phases beyond the point where the unmasked FSC curve
%     falls below a given threshold (by default 0.8) and calculating an
%     additional FSC between these phase randomized maps.  This allows for the
%     determination of the extra correlation caused by effects of the mask,
%     which is then subtracted from the normal masked FSC curves. The curve will
%     be saved as a Matlab figure and a PDF file, and if PLOT_FSC is true it
...
%##############################################################################%
    ...
    addParameter(fn_parser, 'motl_a_fn_prefix', 'even/combinedmotl/allmotl');
    addParameter(fn_parser, 'motl_b_fn_prefix', 'odd/combinedmotl/allmotl');
    addParameter(fn_parser, 'fsc_mask_fn', 'none');
...
%##############################################################################%
    ...
    try
        validateattributes(iteration, {'numeric'}, ...
            ...
            'subtom_maskcorrected_fsc_bfactor', 'iteration');
    ...
    % Since we have to figure out how many subsets we save validation for later.
    ref_a_fn_prefix = fn_parser.Results.ref_a_fn_prefix;
    ref_b_fn_prefix = fn_parser.Results.ref_b_fn_prefix;

    motl_a_fn_prefix = fn_parser.Results.motl_a_fn_prefix;
    motl_a_fn = sprintf('%s_%d.em', motl_a_fn_prefix, iteration);
    
    if exist(motl_a_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'maskcorrected_fsc_bfactor:File %s does not exist.', motl_a_fn);
    ...
    motl_b_fn_prefix = fn_parser.Results.motl_b_fn_prefix;
    motl_b_fn = sprintf('%s_%d.em', motl_b_fn_prefix, iteration);
    
    if exist(motl_b_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'maskcorrected_fsc_bfactor:File %s does not exist.', motl_b_fn);
    ...
    if ~strcmpi(fsc_mask_fn, 'none') && exist(fsc_mask_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'maskcorrected_fsc_bfactor:File %s does not exist.', ...
                fsc_mask_fn);
    ...
    if ~isempty(output_dir) && exist(output_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'maskcorrected_fsc_bfactor:output_dir: Directory %s %s.', ...
    ...
    try
        validateattributes(do_reweight, {'numeric'}, ...
            ...
            'subtom_maskcorrected_fsc_bfactor', 'do_reweight');
    ...
    try
        validateattributes(do_sharpen, {'numeric'}, ...
            {'scalar', 'nonnan', 'binary'}, ...
            'subtom_maskcorrected_fsc_bfactor', 'do_sharpen');
    ...
    try
        validateattributes(plot_fsc, {'numeric'}, ...
            {'scalar', 'nonnan', 'binary'}, ...
            'subtom_maskcorrected_fsc_bfactor', 'plot_fsc');
    ...
    try
        validateattributes(plot_sharpen, {'numeric'}, ...
            {'scalar', 'nonnan', 'binary'}, ...
            'subtom_maskcorrected_fsc_bfactor', 'plot_sharpen');
    ...
    try
        validateattributes(filter_mode, {'numeric'}, ...
            ...
            'subtom_maskcorrected_fsc_bfactor', 'filter_mode');
    ...
    if filter_mode == 1
        try
            validateattributes(filter_threshold, {'numeric'}, ...
                {'scalar', 'nonnan', '>=', 0, '<=', 1}, ...
                'subtom_maskcorrected_fsc_bfactor', 'filter_threshold');
    ...
    try
        validateattributes(pixelsize, {'numeric'}, ...
            {'scalar', 'nonnan', '>=', 0}, ...
            'subtom_maskcorrected_fsc_bfactor', 'pixelsize');
    ...
    try
        validateattributes(nfold, {'numeric'}, ...
            ...
            'subtom_maskcorrected_fsc_bfactor', 'nfold');
    ...
    try
        validateattributes(rand_threshold, {'numeric'}, ...
            {'scalar', 'nonnan', '>=', 0, '<=', 1}, ...
            'subtom_maskcorrected_fsc_bfactor', 'rand_threshold');
    ...
    try
        validateattributes(box_gaussian, {'numeric'}, ...
            ...
            'subtom_maskcorrected_fsc_bfactor', 'box_gaussian');
    ...
    try
        validateattributes(iclass, {'numeric'}, ...
            {'scalar', 'nonnan', 'integer'}, ...
            'subtom_maskcorrected_fsc_bfactor', 'iclass');
...
%##############################################################################%
    ...
    if size(motl_a, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'maskcorrected_fsc_bfactor:%s is not proper MOTL.', motl_a_fn);
    ...
    if size(motl_b, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'maskcorrected_fsc_bfactor:%s is not proper MOTL.', motl_b_fn);
    ...
    num_good_ptcls_b = sum(motl_b(20, :) == 1 | motl_b(20, :) == iclass);
    num_subsets_b = floor(log2(num_good_ptcls_b / 128)) + 1;

    % We set the number of subsets equal to the minimum number of subsets
    % between the two halves. This should most often not be needed but is an
    % edge case for when for example motl_a has 1024 particles and thus 4
    % subsets and motl_b has 1023 particles and only 3 subsets.
    num_subsets = min(num_subsets_a, num_subsets_b);

    % We set the number of good particles in total to the sum of the number of
    % good particles in motive list a and b.
    num_good_ptcls = num_good_ptcls_a + num_good_ptcls_b;

    % Normaly we can just read in the reference a and use that to check file
    % sizes but here it is too complicated with subsets, so just get the size
    % from the header of the first subset of reference a.
    check_fn = sprintf('%s_subset_1_%d.em', ref_a_fn_prefix, iteration);

    if exist(check_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'maskcorrected_fsc_bfactor:File %s does not exist.', check_fn);
    ...
    if ~all(size(fsc_mask) == box_size)
        try
            error('subTOM:volDimError', ...
                'maskcorrected_fsc_bfactor:%s and %s are not same size.', ...
    ...
    % Number of Fourier Shells, hardcoded to half the box-size plus one for
    % the DC (average / infinite wavelength / zero-frequency) component.
    n_shells = cartesian_origin + 1;

    % Initialize the RNG
    rng('shuffle');

    % Create a cell array to hold all of the FSCs and an array to hold all of
    % the resolutions.
    corrected_fsc_cell = cell(1, num_subsets);
    resolutions = zeros(1, num_subsets);

    % Loop over the subsets and calculate the maskcorrected FSC of each
    for subset_idx = 1:num_subsets

        % Read in the references.
        ref_a_fn = sprintf('%s_subset_%d_%d.em', ref_a_fn_prefix, ...
        ...
        if ~all(size(ref_a) == box_size)
            try
                error('subTOM:volDimError', ...
                    'maskcorrected_fsc_bfactor:%s and %s %s.', ...
                    ref_a_fn, check_fn, 'are not same size');
        ...
        ref_b_fn = sprintf('%s_subset_%d_%d.em', ref_b_fn_prefix, ...
            subset_idx, iteration);
        ...
        if ~all(size(ref_b) == box_size)
            try
                error('subTOM:volDimError', ...
                    'maskcorrected_fsc_bfactor:%s and %s %s.', ...
                    ref_b_fn, check_fn, 'are not same size');
        ...
        % Create "unmasked" references to determine FSC phase-randomization
        % cutoffs RELION uses the true unmasked FSC to determine this, but maybe
        % a bad idea for continuous densities like could occur with any lattice
        % structure.
        ...
        %% Sum Fourier shells
        % Normal shell sums
        shell_idxs = arrayfun(@(x) find(round(radii) == (x - 1)), ...
            1:n_shells, 'UniformOutput', false);
        ...
        % references that really throws off the calculations and so this is a
        % small work around to make sure we don't use this artifact as the
        % cutoff.
        ...
        % If the FSC never fell below the threshold we skip the phase
        % randomization since it takes a bit of time to calculate.
        if rand_cutoff
            fprintf(1, 'Phase Randomization at Fourier Pixel: %d\n', ...
                rand_cutoff);

            % Because the map needs to stay centro-symmetric we have to do some
            % ugly code to randomize the phases and this depends on whether or
            % not the box size is odd or even since this affects the number of
            % unique frequencies in the Fourier transform.

            % This should be the indices in the Fourier Transform that belong to
            % a unique half, and special care needs to be taken for one-half of
            % the X-axis.
            rand_idxs = find(round(radii) > (rand_cutoff - 1) & ...
                (cart_x < 0 | cart_x == 0 & cart_y < 0))';

            % This should be the indices that belond to the centro-symmetric
            % version of the indices above.
            ...
            rand_masked_intensity_a = masked_rand_a_fft .* ...
                conj(masked_rand_a_fft);

            rand_masked_intensity_b = masked_rand_b_fft .* ...
                conj(masked_rand_b_fft);
        ...
        % Place the corrected FSC into the FSC cell array.
        corrected_fsc_cell{subset_idx} = corrected_fsc;

        % Interpolate the FSC curve to find where it crosses the user given
        % threshold.

        % First if the filter mode was given as 2 we need to convert the Fourier
        % pixel value to an FSC value. If the filter mode is 1, we just use the
        % given threshold as the point of interest.
        if filter_mode == 1
            fsc_poi = filter_threshold;
        else
            fsc_poi = corrected_fsc_cell{1}(filter_threshold + 1);
        end
            ...
            if isempty(x3)
                try
                    error('subTOM:FSCError', ...
                        'maskcorrected_fsc_bfactor: FSC never above %f!\n', ...
                        fsc_poi);
        ...
        % Add the resolution to the resolutions array.
        resolutions(subset_idx) = resolution_a;
    end
    ...
    grid(fsc_axes, 'on');
    legend(fsc_axes, 'Location', 'eastoutside', 'Orientation', 'vertical');
    legend('boxoff');
    ...
    domain_mean = mean(b_factor_domain(:));
    range_mean = mean(b_factor_range(:));
    domain_variance = sum((b_factor_domain(:) - domain_mean).^2);
    b_factor_covariance = sum((b_factor_domain(:) - domain_mean) .* ...
        (b_factor_range(:) - range_mean));
    ...
    if plot_fsc
        ...
        b_factor_fig = figure('Visible', 'off');
    end
    ...
    if do_sharpen == 1
        ...
        % Read in the references.
        ref_a_fn = sprintf('%s_subset_1_%d.em', ref_a_fn_prefix, iteration);
        ref_a = getfield(tom_emread(ref_a_fn), 'Value');

        ref_b_fn = sprintf('%s_subset_1_%d.em', ref_b_fn_prefix, iteration);
        ref_b = getfield(tom_emread(ref_b_fn), 'Value');
        ...
        if (do_reweight == 1)
            ...
            if ~all(size(filter_a) == box_size)
                try
                    error('subTOM:volDimError', ...
                        'maskcorrected_fsc_bfactor:%s and %s %s.', ...
                        filter_a_fn, check_fn, 'are not same size');
            ...
            if ~all(size(filter_b) == box_size)
                try
                    error('subTOM:volDimError', ...
                        'maskcorrected_fsc_bfactor:%s and %s %s.', ...
                        filter_b_fn, check_fn, 'are not same size');
            ...
            sharp_reweigh = subtom_sharpen('reference', reweigh_ref, ...
                'b_factor', b_factor, 'fsc', corrected_fsc_cell{1}, ...
