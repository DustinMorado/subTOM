...
% SUBTOM_WEIGHTED_AVERAGE joins and weights subsets of average subsets.
%
...
%##############################################################################%
    ...
    for subset_idx = 1:num_subsets
        ref_fn = sprintf('%s_%d_subset%d.em', ref_fn_prefix, iteration, ...
            subset_idx);
    ...
    if all_done
        warning('subTOM:recoverOnFail', ...
            'weighted_average:File %s already exists. SKIPPING!', ref_fn);
    ...
    check_fn = sprintf('%s_%d_1_subset1.em', ref_fn_prefix, iteration);

    if exist(check_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'weighted_average:File %s does not exist.', check_fn);
    ...
    for batch_idx = 1:num_avg_batch
        ...
        % Handle each subset
        for subset_idx = 1:num_subsets
            ref_fn = sprintf('%s_%d_%d_subset%d.em', ref_fn_prefix, ...
            ...
            weight_sum_fn = sprintf('%s_%d_%d_subset%d.em', ...
                weight_sum_fn_prefix, iteration, batch_idx, subset_idx);

            if exist(weight_sum_fn, 'file') ~= 2
                try
                    error('subTOM:missingFileError', ...
                        'weighted_average:File %s does not exist.', ...
                        weight_sum_fn);
    ...
    for subset_idx = 1:num_subsets
        ...
        average_raw_fn = sprintf('%s_debug_raw_%d_subset%d.em', ...
            ref_fn_prefix, iteration, subset_idx);
        ...
        weight_average_fn = sprintf('%s_debug_%d_subset%d.em', ...
            weight_sum_fn_prefix, iteration, subset_idx);
