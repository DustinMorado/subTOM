function subtom_compare_motls_multiref(varargin)
% SUBTOM_COMPARE_MOTLS_MULTIREF compares orientations and shifts of two MOTLS.
%
...
%##############################################################################%
    ...
    if isempty(motl_1_fn)
        try
            error('subTOM:missingRequired', ...
                'compare_motls_multiref:Parameter %s is required.', ...
                'motl_1_fn');
    ...
    if exist(motl_1_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'compare_motls_multiref:File %s does not exist.', motl_1_fn);
        catch ME
            fprintf(2, '%s - %s\n', ME.identifier, ME.message);
            rethrow(ME);
        end
    ...
    if size(motl_1, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'compare_motls_multiref:%s is not proper MOTL.', motl_1_fn);
    ...
    if exist(motl_2_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'compare_motls_multiref:File %s does not exist.', motl_2_fn);
        catch ME
            fprintf(2, '%s - %s\n', ME.identifier, ME.message);
            rethrow(ME);
        end
    ...
    if size(motl_2, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'compare_motls_multiref:%s is not proper MOTL.', motl_2_fn);
    ...
    if ~all(motl_1(4, :) == motl_2(4, :))
        try
            error('subTOM:argumentError', ...
                'compare_motls_multiref:%s and %s are not same sized.', ...
                    motl_1_fn, motl_2_fn);
        catch ME
            fprintf(2, '%s - %s\n', ME.identifier, ME.message);
            rethrow(ME);
    ...
    try
        validateattributes(write_diffs, {'numeric'}, ...
            ...
            'subtom_compare_motls_multiref', 'write_diffs');
    ...
    if ~write_diffs && ~isempty(output_diffs_fn)
        try
            error('subTOM:argumentError', ...
                'compare_motls_multiref: %s specified, but not %s', ...
        ...
        if fid == -1
            try
                error('subTOM:writeFileError', ...
                    'compare_motls_multiref:File %s %s.', ...
                    output_diffs_fn, 'cannot be opened for writing');
...
%##############################################################################%
    ...
    class_changes = sum(arrayfun(@(x) motl_1(20, x) ~= motl_2(20, x), ...
        1:n_idxs));

    fprintf(1, '\n%-60s : %d\n', 'Number of class changes', class_changes);

    if write_diffs
        diffs = zeros(8, n_idxs);
        diffs(1, :) = motl_1(4, :);
        diffs(2, :) = motl_1(20, :);
        diffs(3, :) = motl_2(20, :);
        diffs(4, :) = motl_1(1, :);
        diffs(5, :) = motl_2(1, :);
        diffs(6, :) = coord_diffs;
        diffs(7, :) = angular_diffs;
        diffs(8, :) = angular_diffs_no_inplane;
        ...
        fprintf(output_diffs_fd, '%f,%f,%f,%f,%f,%d\n', mean_CCC_2, ...
            median_CCC_2, std_CCC_2, min_CCC_2, max_CCC_2, class_changes);
