function subtom_parallel_xmatrix(varargin)
% SUBTOM_PARALLEL_XMATRIX calculates chunks of the X-matrix for processing.
%
...
%     XMATRIX_FN_PREFIX_ITERATION_PROCESS_IDX.em. The location of the particles
%     is specified by PTCL_FN_PREFIX. If PREALIGNED evaluates to true as a
%     boolean then the particles are assumed to be prealigned, which should
...
%##############################################################################%
    ...
    if ~isempty(all_motl_dir) && exist(all_motl_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_xmatrix:all_motl_dir: Directory %s %s.', ...
    ...
    if ~isempty(xmatrix_dir) && exist(xmatrix_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_xmatrix:xmatrix_dir: Directory %s %s.', ...
    ...
    if ~isempty(ptcl_dir) && exist(ptcl_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_xmatrix:ptcl_dir: Directory %s %s.', ...
    ...
    try
        validateattributes(iteration, {'numeric'}, ...
            ...
            'subtom_parallel_xmatrix', 'iteration');
    ...
    try
        validateattributes(prealigned, {'numeric'}, ...
            ...
            'subtom_parallel_xmatrix', 'prealigned');
    ...
    try
        validateattributes(num_xmatrix_batch, {'numeric'}, ...
            ...
            'subtom_parallel_xmatrix', 'num_xmatrix_batch');
    ...
    try
        validateattributes(process_idx, {'numeric'}, ...
            {'scalar', 'nonnan', 'integer', '>', 0, ...
            '<=', num_xmatrix_batch}, ...
            'subtom_parallel_xmatrix', 'process_idx');
...
%##############################################################################%
    ...
    if exist(xmatrix_fn, 'file') == 2
        warning('subTOM:recoverOnFail', ...
            'subtom_parallel_xmatrix:File %s already exists. SKIPPING!', ...
            xmatrix_fn);
    ...
    if exist(all_motl_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'parallel_xmatrix:File %s does not exist.', all_motl_fn);
    ...
    if size(all_motl, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'parallel_xmatrix:%s is not proper MOTL.', all_motl_fn);
    ...
    if num_xmatrix_batch > num_ptcls
        try
            error('subTOM:argumentError', ...
                'parallel_xmatrix:num_xmatrix_batch is %s.', ...
    ...
    if exist(check_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'parallel_xmatrix:File %s does not exist.', check_fn);
        ...
        if ~all(size(mask) == box_size)
            try
                error('subTOM:volDimError', ...
                    'parallel_xmatrix:%s and %s are not same size.', ...
                    mask_fn, check_fn);
    ...
    for ptcl_idx = ptcl_start_idx:ptcl_end_idx
        ...
        if exist(ptcl_fn, 'file') ~= 2
            try
                error('subTOM:missingFileError', ...
                    'parallel_xmatrix:File %s does not exist.', ptcl_fn);
