function subtom_join_eigencoeffs(varargin)
% SUBTOM_JOIN_EIGENCOEFFS combines chunks of coefficients into the final matrix.
%
...
%##############################################################################%
    ...
    if ~isempty(eig_coeff_dir) && exist(eig_coeff_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'join_eigencoeffs:eig_coeff_dir: Directory %s %s.', ...
    ...
    try
        validateattributes(iteration, {'numeric'}, ...
            ...
            'subtom_join_eigencoeffs', 'iteration');
    ...
    try
        validateattributes(num_xmatrix_batch, {'numeric'}, ...
            ...
            'subtom_join_eigencoeffs', 'num_xmatrix_batch');
...
%##############################################################################%
    ...
    if exist(eig_coeff_fn, 'file') == 2
        warning('subTOM:recoverOnFail', ...
            'subtom_join_eigencoeffs:File %s already exists. SKIPPING!', ...
            eig_coeff_fn);
    ...
    if exist(check_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'join_eigencoeffs:File %s does not exist.', check_fn);
    ...
    for batch_idx = 2:num_xmatrix_batch
        ...
        if exist(eig_coeff_fn, 'file') ~= 2
            try
                error('subTOM:missingFileError', ...
                    'join_eigencoeffs:File %s does not exist.', ...
                    eig_coeff_fn);
        ...
        if batch_size(2) ~= num_eigs
            try
                error('subTOM:volDimError', ...
                    'join_eigencoeffs:%s and %s are not same size.', ...
