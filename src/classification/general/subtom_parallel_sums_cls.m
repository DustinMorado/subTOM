function subtom_parallel_sums_cls(varargin)
% SUBTOM_PARALLEL_SUMS_CLS creates raw sums and Fourier weight sums in a batch.
%
...
%
% See also SUBTOM_WEIGHTED_AVERAGE_CLS
...
%##############################################################################%
    ...
    if ~isempty(all_motl_dir) && exist(all_motl_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_sums_cls:all_motl_dir: Directory %s %s.', ...
    ...
    if ~isempty(ref_dir) && exist(ref_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_sums_cls:ref_dir: Directory %s %s.', ...
    ...
    if ~isempty(ptcl_dir) && exist(ptcl_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_sums_cls:ptcl_dir: Directory %s %s.', ...
    ...
    if ~isempty(weight_dir) && exist(weight_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_sums_cls:weight_dir: Directory %s %s.', ...
    ...
    if ~isempty(weight_sum_dir) && exist(weight_sum_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'parallel_sums_cls:weight_sum_dir: Directory %s %s.', ...
    ...
    try
        validateattributes(iteration, {'numeric'}, ...
            ...
            'subtom_parallel_sums_cls', 'iteration');
    ...
    try
        validateattributes(tomo_row, {'numeric'}, ...
            ...
            'subtom_parallel_sums_cls', 'tomo_row');
    ...
    try
        validateattributes(num_avg_batch, {'numeric'}, ...
            ...
            'subtom_parallel_sums_cls', 'num_avg_batch');
    ...
    try
        validateattributes(process_idx, {'numeric'}, ...
            ...
            'subtom_parallel_sums_cls', 'process_idx');
...
%##############################################################################%
    ...
    if exist(all_motl_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'parallel_sums_cls:File %s does not exist.', all_motl_fn);
    ...
    if size(all_motl, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'parallel_sums_cls:%s is not proper MOTL.', all_motl_fn);
    ...
    if all_complete
        warning('subTOM:recoverOnFail', ...
            'parallel_sums_cls:File %s already exists. SKIPPING!', ref_fn);
    ...
    if exist(ptcl_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'parallel_sums_cls:File %s does not exist.', ptcl_fn);
    ...
    for ptcl_idx = ptcl_start_idx:ptcl_end_idx
        ...
        if ~all(size(ptcl) == box_size)
            try
                error('subTOM:volDimError', ...
                    'parallel_sums_cls:%s and %s are not same size.', ...
                    ptcl_fn, check_fn);
        ...
        if current_weight ~= all_motl(tomo_row, ptcl_idx)
            ...
            if exist(weight_fn, 'file') ~= 2
                try
                    error('subTOM:missingFileError', ...
                        'parallel_sums_cls:File %s does not exist.', weight_fn);
