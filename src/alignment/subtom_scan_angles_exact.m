function subtom_scan_angles_exact(varargin)
% SUBTOM_SCAN_ANGLES_EXACT align a particle batch over local search angles.
%
...
%##############################################################################%
    ...
    if ~isempty(all_motl_dir) && exist(all_motl_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'scan_angles_exact:all_motl_dir: Directory %s %s.', ...
    ...
    if ~isempty(ref_dir) && exist(ref_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'scan_angles_exact:ref_dir: Directory %s %s.', ...
    ...
    if ~isempty(ptcl_dir) && exist(ptcl_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'scan_angles_exact:ptcl_dir: Directory %s %s.', ...
    ...
    if exist(weight_dir, 'dir') ~= 7
        try
            error('subTOM:missingDirectoryError', ...
                'scan_angles_exact:weight_dir: Directory %s %s.', ...
    ...
    if ~strcmp(align_mask_fn, 'none') && exist(align_mask_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'scan_angles_exact:File %s does not exist.', align_mask_fn);
    ...
    if ~strcmp(cc_mask_fn, 'noshift') && exist(cc_mask_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'scan_angles_exact:File %s does not exist.', cc_mask_fn);
    ...
    try
        validateattributes(apply_weight, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'apply_weight');
    ...
    try
        validateattributes(psi_angle_step, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'psi_angle_step');
    ...
    try
        validateattributes(psi_angle_shells, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'psi_angle_shells');
    ...
    try
        validateattributes(phi_angle_step, {'numeric'}, ...
            {'scalar', 'nonnan', 'nonnegative'}, ...
            'subtom_scan_angles_exact', 'phi_angle_step');
    ...
    try
        validateattributes(phi_angle_shells, {'numeric'}, ...
            {'scalar', 'nonnan', 'nonnegative', 'integer'}, ...
            'subtom_scan_angles_exact', 'phi_angle_shells');
    ...
    try
        validateattributes(high_pass_fp, {'numeric'}, ...
            {'scalar', 'nonnan', 'nonnegative'}, ...
            'subtom_scan_angles_exact', 'high_pass_fp');
    ...
    try
        validateattributes(high_pass_sigma, {'numeric'}, ...
            {'scalar', 'nonnan', 'nonnegative'}, ...
            'subtom_scan_angles_exact', 'high_pass_sigma');
    ...
    try
        validateattributes(low_pass_fp, {'numeric'}, ...
            {'scalar', 'nonnan', 'nonnegative'}, ...
            'subtom_scan_angles_exact', 'low_pass_fp');
    ...
    try
        validateattributes(low_pass_sigma, {'numeric'}, ...
            {'scalar', 'nonnan', 'nonnegative'}, ...
            'subtom_scan_angles_exact', 'low_pass_sigma');
    ...
    try
        validateattributes(nfold, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'nfold');
    ...
    try
        validateattributes(threshold, {'numeric'}, ...
            {'scalar', 'nonnan', '>=', -1, '<=', 1}, ...
            'subtom_scan_angles_exact', 'threshold');
    ...
    try
        validateattributes(iteration, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'iteration');
    ...
    try
        validateattributes(tomo_row, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'tomo_row');
    ...
    try
        validateattributes(iclass, {'numeric'}, ...
            {'scalar', 'nonnan', 'integer'}, ...
            'subtom_scan_angles_exact', 'iclass');
    ...
    try
        validateattributes(num_ali_batch, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'num_ali_batch');
    ...
    try
        validateattributes(process_idx, {'numeric'}, ...
            ...
            'subtom_scan_angles_exact', 'process_idx');
...
%##############################################################################%
    ...
    if exist(ref_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'scan_angles_exact:File %s does not exist.', ref_fn);
    ...
    if exist(all_motl_fn, 'file') ~= 2
        try
            error('subTOM:missingFileError', ...
                'scan_angles_exact:File %s does not exist.', all_motl_fn);
    ...
    if size(all_motl, 1) ~= 20
        try
            error('subTOM:MOTLError', ...
                'scan_angles_exact:%s is not proper MOTL.', all_motl_fn);
    ...
    if num_ali_batch > num_ptcls
        try
            error('subTOM:argumentError', ...
                'scan_angles_exact:num_ali_batch is greater than num_ptcls.');
    ...
    for ptcl_idx = ptcl_start_idx:ptcl_end_idx
        ...
        if     all_motl(20, ptcl_idx) ~= 1 ...
            && all_motl(20, ptcl_idx) ~= 2 ...
            && all_motl(20, ptcl_idx) ~= iclass ...
        ...
        if current_weight ~= all_motl(tomo_row, ptcl_idx)
            ...
            if exist(weight_fn, 'file') ~= 2
                try
                    error('subTOM:missingFileError', ...
                        'scan_angles_exact:File %s does not exist.', weight_fn);
        ...
        if exist(ptcl_fn, 'file') ~= 2
            try
                error('subTOM:missingFileError', ...
                    'scan_angles_exact:File %s does not exist.', ptcl_fn);
            ...
            for theta_idx = 0:theta_angle_shells
                ...
                for psi_idx = psi_range
                    ...
                    if ccc > max_ccc
                        ...
                        % Normalise the angles so that phi and psi are in the
                        % range (-180, 180] and theta is in the range [0, 180]
                        if theta < 0
                            theta = -theta;
                            psi = psi + 180;
                        end
