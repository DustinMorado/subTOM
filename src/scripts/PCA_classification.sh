#!/bin/bash
################################################################################
# A script for performing PCA classification on a dataset. Most steps are
# performed on the cluster, but the PCA step can be performed locally, as this
# is not parallelizable. The PCA classification method is essentially that found
# in Dynamo.
#
# NOTE: xmatrix files are NOT deleted. They can take up quite a bit of space
# so delete them as necessary.
#
# This subtomogram processing script uses nine MATLAB compiled scripts below:
# - will_lsf_prepare_ccmatrix_sge
# - will_lsf_final_ccmatrix
# - will_pca_ccmatrix_function
# - will_lsf_xmatrix_parallel
# - will_lsf_parallel_eigenvol2
# - will_lsf_parallel_eigencoeff
# - will_lsf_final_eigencoeff
# - will_kmeans_eigenvalues_function
# - will_kmeans_find_optimum_function
# WW 09-2016
################################################################################
...
prep_CCM_exec=${exec_dir}/prepare_ccmatrix

# Finalize CC-Matrix executable
CCM_exec=${exec_dir}/final_ccmatrix

# PCA on CC-Matrix executable
PCA_exec=${exec_dir}/pca_ccmatrix

# Parallel X-Matrix executable
paral_XM_exec=${exec_dir}/parallel_xmatrix

# Parallel Eigenvolume executable
paral_eigenvol_exec=${exec_dir}/parallel_eigenvolume

# Parallel Eigencoefficient executable
paral_eigencoeff_exec=${exec_dir}/parallel_eigencoeff

# Finalize Eigencoefficient executable
eigencoeff_exec=${exec_dir}/final_eigencoeff

# K-Means by Eigenvalues executable
kmean_eigen_exec=${exec_dir}/kmeans_eigenvalues

# K-Means find Optimum executable
kmean_optimum_exec=${exec_dir}/kmeans_optimum
...
### File Options ###
# Index of iteration to use.
ind=1                                                       
# Relative path and name of allmotl file. Give it the complete file name.
allmotlname='./combinedmotl/allmotl-subsetforpca'                 
# Relative path and name of wedgelist.
wedgelistname='./otherinputs/wedgelist.em'                  
# Relative path and name of non-binary wedgemask. Set to 'none' to use standard
# binary mask.
wedgemaskname='none'                                        
# Row number for tomogram number.
tomonum=7                                                   
# Relative path and name of subtomograms.
subtomoname='./subtomograms/subtomo'                        
# Relative path and name of classification mask. Setting to 'none' results in
# the use of a spherical mask.
maskname='./otherinputs/pcamask_hexneig_sph_r20_h20.em'                      
# Folder for temporary files. There are deleted after the calculations are over.
temp_folder='./pca/'                                        

### CC-matrix Calculation ###
# Calculate CC-matrix. 1 = yes, 0 = no.
calculate_ccmatrix=1                    
# Number of cores to use for CC-matrix calculations. KEEP THIS LIMITED IF
# RUNNING FROM GROUPSHARES
n_cores_ccmatrix=500                   
# Low pass filter in Fourier pixels
lowpass=13                              
# Low pass filter dropoff in Fourier pixels
lpsigma=3                               
# High pass filter in Fourier pixels
hipass=1                                
# High pass filter dropoff in Fourier pixels
hpsigma=2                               
# Relative path and name of CC-matrix. Give it the complete file name.
ccmatrixname='./pca/ccmatrix'           
...
mem_free='2G'                 # The amount of memory your job requires
mem_max='3G'                 # The upper bound on the amount of memory your job is allowed to use
...
if [ $calculate_ccmatrix -eq 1 ]; then
    ...
    if [ ! -f $compdir/prep_cc_matrix ]; then
        ...
        echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> prepare_ccmatrix
        #echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> prepare_ccmatrix
        echo "cd $scratch_dir" >> prepare_ccmatrix
        echo "rm -rf $scratch_dir${mcr_cache_dir}/prepare_ccmatrix/" >> prepare_ccmatrix
        echo "mkdir $scratch_dir${mcr_cache_dir}/prepare_ccmatrix/" >> prepare_ccmatrix
        echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/prepare_ccmatrix/" >> prepare_ccmatrix
          echo "time /net/bstore1/bstore1/briggsgrp/atan/software/Matlab_scripts/cluster_scripts/sge/compile/pca_tools/ccmatrix/will_lsf_prepare_ccmatrix_sge $allmotlname $ccmatrixname $ind $wedgelistname $wedgemaskname $tomonum $subtomoname $maskname $lowpass $lpsigma $hipass $hpsigma $mem_free $mem_max $n_cores_ccmatrix $temp_folder $check_ccmatrix_paral" >> prepare_ccmatrix
        echo "rm -rf $scratch_dir${mcr_cache_dir}/prepare_ccmatrix/" >> prepare_ccmatrix
    ...
    if [ ! -f $compdir/final_cc_matrix ]; then
        ...
        echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> ccmatrix_Final
        echo "cd $scratch_dir" >> ccmatrix_Final
        echo "rm -rf $scratch_dir${mcr_cache_dir}/ccmatrix_Final/" >> ccmatrix_Final
        echo "mkdir $scratch_dir${mcr_cache_dir}/ccmatrix_Final/" >> ccmatrix_Final
        echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}//ccmatrix_Final/" >> ccmatrix_Final
...
if [ $calculate_pca -eq 1 ]; then
    ...
    if [ ! -f $compdir/pca_calc ]; then
        ...
        if [ $local_pca != 1 ]; then
            ...
            echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> calculate_PCA
            echo "cd $scratch_dir" >> calculate_PCA
            echo "rm -rf $scratch_dir${mcr_cache_dir}/calculate_PCA/" >> calculate_PCA
            echo "mkdir $scratch_dir${mcr_cache_dir}/calculate_PCA/" >> calculate_PCA
            echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/calculate_PCA/" >> calculate_PCA
        ...
        # Calculate via local_dummy.sh
            ...
            echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> ${local_dir}/calculate_PCA
            echo "cd ${local_dir}/" >> ${local_dir}/calculate_PCA
            echo "rm -rf ${local_dir}${mcr_cache_dir}/calculate_PCA/" >> ${local_dir}/calculate_PCA
            echo "rm -rf ${local_dir}/checkjobs/" >> ${local_dir}/calculate_PCA
            echo "mkdir checkjobs" >> ${local_dir}/calculate_PCA
            echo "mkdir -p ${local_dir}/${mcr_cache_dir}/calculate_PCA/" >> ${local_dir}/calculate_PCA
            echo export MCR_CACHE_ROOT="${local_dir}${mcr_cache_dir}/calculate_PCA/" >> ${local_dir}/calculate_PCA
...
if [ $calculate_xmatrix -eq 1 ]; then
    if [ ! -f $compdir/paral_xmatrix ]; then
        ...
        while [ $d -le $n_cores_xmatrix ]; do
            ...
            echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> parallel_xmatrix_$procnum
            echo "cd $scratch_dir" >> parallel_xmatrix_$procnum
            echo "rm -rf $scratch_dir${mcr_cache_dir}/parallel_xmatrix_$procnum/" >> parallel_xmatrix_$procnum
            echo "mkdir $scratch_dir${mcr_cache_dir}/parallel_xmatrix_$procnum/" >> parallel_xmatrix_$procnum
            echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/parallel_xmatrix_$procnum/" >> parallel_xmatrix_$procnum
...
if [ $calculate_eigenvolumes -eq 1 ]; then
    ...
    if [ ! -f $compdir/paral_eigenvolume ]; then
        ...
        while [ $d -le $n_cores_xmatrix ]; do
            ...
            echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> parallel_eigenvolume_$procnum
            echo "cd $scratch_dir" >> parallel_eigenvolume_$procnum
            echo "rm -rf $scratch_dir${mcr_cache_dir}/parallel_eigenvolume_$procnum/" >> parallel_eigenvolume_$procnum
            echo "mkdir $scratch_dir${mcr_cache_dir}/parallel_eigenvolume_$procnum/" >> parallel_eigenvolume_$procnum
            echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/parallel_eigenvolume_$procnum/" >> parallel_eigenvolume_$procnum
    ...
    if [ ! -f $compdir/final_eigenvol ]; then
        ...
        echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> eigenvolume_Final
        echo "cd $scratch_dir" >> eigenvolume_Final
        echo "rm -rf $scratch_dir${mcr_cache_dir}/eigenvolume_Final/" >> eigenvolume_Final
        echo "mkdir $scratch_dir${mcr_cache_dir}/eigenvolume_Final/" >> eigenvolume_Final
        echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/eigenvolume_Final/" >> eigenvolume_Final
...
if [ $calculate_eigencoeff -eq 1 ]; then
    ...
    if [ ! -f $compdir/paral_eigencoeff ]; then
        ...
        while [ $d -le $n_cores_xmatrix ]; do
            ...
            echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> parallel_eigencoeff_$procnum
            echo "cd $scratch_dir" >> parallel_eigencoeff_$procnum
            echo "rm -rf $scratch_dir${mcr_cache_dir}/parallel_eigencoeff_$procnum/" >> parallel_eigencoeff_$procnum
            echo "mkdir $scratch_dir${mcr_cache_dir}/parallel_eigencoeff_$procnum/" >> parallel_eigencoeff_$procnum
            echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/parallel_eigencoeff_$procnum" >> parallel_eigencoeff_$procnum
    ...
    if [ ! -f $compdir/final_eigencoeff ]; then
        ...
        echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> eigencoeff_Final
        echo "cd $scratch_dir" >> eigencoeff_Final
        echo "rm -rf $scratch_dir${mcr_cache_dir}/eigencoeff_Final/" >> eigencoeff_Final
        echo "mkdir $scratch_dir${mcr_cache_dir}/eigencoeff_Final/" >> eigencoeff_Final
        echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/eigencoeff_Final/" >> eigencoeff_Final
...
if [ $calculate_kmeans -eq 1 ]; then
    ...
    if [ ! -f $compdir/paral_kmeans ]; then
        ...
        while [ $d -le $replicates ]; do
            ...
            echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> parallel_kmeans_$procnum
            echo "cd $scratch_dir" >> parallel_kmeans_$procnum
            echo "rm -rf $scratch_dir${mcr_cache_dir}/parallel_kmeans_$procnum/" >> parallel_kmeans_$procnum
            echo "mkdir $scratch_dir${mcr_cache_dir}/parallel_kmeans_$procnum/" >> parallel_kmeans_$procnum
            echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/parallel_kmeans_$procnum" >> parallel_kmeans_$procnum
    ...
    if [ ! -f $compdir/final_kmeans ]; then
        ...
        echo 'export LD_LIBRARY_PATH="XXXMCR_DIRXXX/runtime/glnxa64:XXXMCR_DIRXXX/bin/glnxa64:XXXMCR_DIRXXX/sys/os/glnxa64:XXXMCR_DIRXXX/sys/opengl/lib/glnxa64"' >> kmean_Final
        echo "cd $scratch_dir" >> kmean_Final
        echo "rm -rf $scratch_dir${mcr_cache_dir}/kmean_Final/" >> kmean_Final
        echo "mkdir $scratch_dir${mcr_cache_dir}/kmean_Final/" >> kmean_Final
        echo export MCR_CACHE_ROOT="$scratch_dir${mcr_cache_dir}/kmean_Final/" >> kmean_Final
