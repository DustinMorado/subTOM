...
if [[ "${num_xmatrix_prealign_batch}" -gt "${max_jobs}" ]]
then
...
################################################################################
if [[ "${xmatrix_prealign}" -eq "1" ]]
then

    # Calculate number of job scripts needed
    num_jobs=$(((num_xmatrix_prealign_batch + array_max - 1) / array_max))
    job_name_="${job_name}_xmatrix_parallel_prealign"
    ...
    do
        ...
        if [[ ${array_end} -gt ${num_xmatrix_prealign_batch} ]]
        then
            array_end=${num_xmatrix_prealign_batch}
        fi
...
###for SGE_TASK_ID in {${array_start}..${array_end}}; do
    ...
    "${preali_exec}" \\
        all_motl_fn_prefix \\
        "${scratch_dir}/${xmatrix_all_motl_fn_prefix}" \\
        ptcl_fn_prefix \\
...
PXPREALIJOB
    ...
    all_motl_fn="${scratch_dir}/${xmatrix_all_motl_fn_prefix}_${iteration}.em"
    ...
    then
        echo -e "\nSTARTING X-Matrix Prealignment - Iteration: ${iteration}\n"
        ...
        echo -e "\nSKIPPING X-Matrix Prealignment - Iteration: ${iteration}\n"
    fi
...
################################################################################
    ...
    do
        ...
        then
            echo "Parallel X-Matrix prealignment has seemed to stall"
...
################################################################################
    if [[ ! -d msa_${iteration} ]]
    then
        mkdir msa_${iteration}
    fi

    if [[ -e "${job_name_}_1" ]]
    then
        mv -f "${job_name_}"_* msa_${iteration}/.
    fi

    if [[ -e "log_${job_name_}_1" ]]
    then
        mv -f "log_${job_name_}"_* msa_${iteration}/.
    fi

    if [[ -e "error_${job_name_}_1" ]]
    then
        mv -f "error_${job_name_}"_* msa_${iteration}/.
    fi

    find "${mcr_cache_dir}" -regex ".*/${job_name_}_[0-9]+" -print0 |\
        xargs -0 -I {} rm -rf -- {}

    echo -e "\nFINISHED X-Matrix Prealignment - Iteration: ${iteration}\n"
fi
...
job_name_="${job_name}_parallel_xmatrix_msa"
...
xmatrix_fn="${scratch_dir}/${xmatrix_fn_prefix}_${iteration}.em"
...
then
    ...
    job_name_="${job_name}_join_xmatrix_msa"
...
then
    ...
    find "${xmatrix_dir}" -regex \
        ".*/${xmatrix_base}_${iteration}.em" -print0 |\
        xargs -0 -I {} cp -- {} "${local_xmatrix_dir}/."

    find "${xmatrix_dir}" -regex \
        ".*/${xmatrix_base}_mean_${iteration}.em" -print0 |\
        xargs -0 -I {} cp -- {} "${local_xmatrix_dir}/."

fi

find "${xmatrix_dir}" -regex \
    ".*/${xmatrix_base}_${iteration}_[0-9]+.em" -delete
...
then
    ...
    job_name_="${job_name}_eigenvolumes_msa"
    mcr_cache_dir_="${mcr_cache_dir}/${job_name_}"
...
job_name_="${job_name}_parallel_eigencoeffs_msa"
...
then
    ...
    job_name_="${job_name}_join_eigencoeffs_msa"
    mcr_cache_dir_="${mcr_cache_dir}/${job_name_}"
...
printf "# MSA Classification Iteration %d\n" "${iteration}" >>\
    subTOM_protocol.md
...
printf "| %-25s | %25s |\n" "par_xmatrix_exec" "${par_xmatrix_exec}" >>\
    subTOM_protocol.md
...
printf "| %-25s | %25s |\n" "num_xmatrix_prealign_batch" \
    "${num_xmatrix_prealign_batch}" >> subTOM_protocol.md
...
printf "| %-25s | %25s |\n" "xmatrix_prealign" "${xmatrix_prealign}" >>\
    subTOM_protocol.md

printf "| %-25s | %25s |\n" "xmatrix_all_motl_fn_prefix" \
    "${xmatrix_all_motl_fn_prefix}" >> subTOM_protocol.md
