function lmb_maskcorrected_FSC(reference_A_fn, reference_B_fn, FSC_mask_fn, ...
...
% LMB_MASKCORRECTED_FSC calculates "mask-corrected" FSC and sharpened volumes.
% LMB_MASKCORRECTED_FSC(REFERENCE_A_FN, REFERENCE_B_FN, FSC_MASK_FN,
%    OUTPUT_FN_PREFIX, PIXELSIZE, NFOLD, RAND_THRESHOLD, PLOT_FSC, DO_SHARPEN,
%    B_FACTOR, BOX_GAUSSIAN, FILTER_MODE, FILTER_THRESHOLD, PLOT_SHARPEN,
%    DO_REWEIGHT, FILTER_A_FN, FILTER_B_FN) takes in two references
%    REFERENCE_A_FN and REFERENCE_B_FN and a FSC mask FSC_MASK_FN and calculates
...
%    and a pdf file, and if PLOT_FSC is true it will also be displayed. The
%    script can also output maps with the prefix OUTPUT_FN_PREFIX that have been
%    sharpened with B_FACTOR if DO_SHARPEN is turned on. This setting has two
%    threshold settings selected using FILTER_MODE, FSC (1) and pixel (2). FSC
%    allows you to use a FSC value FILTER_THRESHOLD as a cutoff for the lowpass
%    filter, while using pixels allows you to use an arbitrary resolution cutoff
%    in FILTER_THRESHOLD. The sharpening curve will be saved as a Matlab figure
%    and a pdf file, and if PLOT_SHARPEN is true it will also be displayed.
...
%
% Example: LMB_MASKCORRECTED_FSC('even/ref/ref_1.em', 'odd/ref/ref_1.em', ...
...
%   be displayed. The map will be filtered using the mask-corrected curve where
%   it falls below 0.143. The map will also be reweighted using the given
%   Fourier reweighting volumes. The function will write out the following
%   files:
%       * 'ref_1_unsharpref.em' - The joined map with no sharpening applied
%       * 'ref_1_finalsharpref_100.em' - The joined map sharpened with a
%           B-Factor of -100.
%       * 'ref_1_finalsharpref_100_reweight.em' - The joined map sharpened with
%           a B-Factor of -100 and reweighted.
...
%       * 'ref_1_sharp.fig' - The curve  used to FSC-filter and sharpen the map
%           in Matlab figure format.
%       * 'ref_1_sharp.pdf' - The curve  used to FSC-filter and sharpen the map
%           in PDF format.
...
% Complex conjugate product - corresponding to the Cross-Correlation Function
...
% Hardcoded to half the box-size
n_shells = boxsize / 2;
...
for shell_idx = 1:n_shells
    ...
    unmasked_CCF_array(shell_idx) = sum(unmasked_CCF(shell_mask{shell_idx}));
...
randomized_A_fft = reference_A_amplitude .* exp(randomized_A_phase * sqrt(-1));
randomized_B_fft = reference_B_amplitude .* exp(randomized_B_phase * sqrt(-1));
...
for FSC_POI = [0.5, 0.143]
    ...
    % Handle the case in low binnings when the FSC never falls below threshold
    if isempty(x2)
...
plot(1:size(unmasked_FSC, 2), unmasked_FSC, 'LineStyle', ':', 'Color', 'b', ...
    'DisplayName', 'Unmasked FSC');
...
if do_sharpen == 1
    ...
    if (do_reweight == 1)
        ...
        % Inverse FFT
        reweighted_reference_A = ifftn(ifftshift(reweighted_reference_A_fft));
        ...
        % Inverse FFT
        reweighted_reference_B = ifftn(ifftshift(reweighted_reference_B_fft));

        if box_gaussian ~= 0
            smoothed_reweighted_A = smooth_box_edge(reweighted_reference_A, ...
                box_gaussian);

            smoothed_reweighted_B = smooth_box_edge(reweighted_reference_B, ...
                box_gaussian);
        ...
        % Write out sharpened reference
        sharpened_reweighted_fn = sprintf('%s_finalsharpref_%d_reweight.em', ...
            output_fn_prefix, (B_factor * -1));
...
%% A function for sharpening a density map
